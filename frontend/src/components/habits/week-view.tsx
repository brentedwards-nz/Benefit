...
interface WeekViewProps {
    ...
    isSelf?: boolean;
    onBack: () => void;
    onHabitToggle: (programmeHabitId: string, date: Date, completed: boolean) => void;
}
...
export const WeekView = ({
    ...
    const todayOnlyTs = new Date(new Date().toISOString().split('T')[0]).getTime();
    const selectedOnlyTs = selectedDay ? new Date(selectedDay.toISOString().split('T')[0]).getTime() : null;
    const isFutureSelectedDay = selectedOnlyTs !== null ? selectedOnlyTs > todayOnlyTs : false;
    const isMoreThan3DaysPast = selectedOnlyTs !== null ? (todayOnlyTs - selectedOnlyTs) / 86400000 > 3 : false;
    ...
    const isWithinProgramme = (programmeHabit: ProgrammeHabit, date: Date) => {
        const habitStartDate = programmeHabit.programme.startDate ? new Date(programmeHabit.programme.startDate) : null;
        const habitEndDate = programmeHabit.programme.endDate ? new Date(programmeHabit.programme.endDate) : null;
            ...
            <div className="mb-4">
                <Button
                    ...
                    onClick={() => {
                        ...
                        window.location.href = `/dashboard/client/habits${queryParams.toString() ? `?${queryParams.toString()}` : ''}`;
                    }}
                ...
                <h1 className="text-2xl text-center font-bold">
                    {selectedWeek.days[0]?.date.toLocaleDateString('en-US', {
                        month: 'short',
                        day: 'numeric'
                    })} - {selectedWeek.days[selectedWeek.days.length - 1]?.date.toLocaleDateString('en-US', {
                        month: 'short',
                        day: 'numeric'
                    })}
                </h1>
            ...
            <div className="mb-6 flex justify-center">
                <div className="grid grid-cols-7 gap-2 w-full max-w-xl">
                    {selectedWeek.days.map((day, dayIndex) => {
                        ...
                        const activeHabits = programmeHabits.filter(ph => isWithinProgramme(ph, day.date));
                        ...
                        return (
                            <div
                                key={dayIndex}
                                className={`flex flex-col items-center gap-1 transition-all duration-200 ${withinProgramme ? (isSelected ? 'scale-110 cursor-pointer' : 'hover:scale-105 cursor-pointer') : 'opacity-60 cursor-not-allowed'}`}
                                onClick={() => {
                                    if (withinProgramme) {
                            ...
                            >
                                {/* Day label above */}
                                <div className={`text-xs font-medium text-center ${isSelected ? 'text-blue-600' : 'text-muted-foreground'
                                    }`}>
                    ...
                    <CardContent>
                        {programmeHabits.length > 0 ? (
                            <div className="space-y-3">
                                {programmeHabits
                                    .filter(h => {
                                        const start = h.programme?.startDate ? new Date(h.programme.startDate) : null;
                                        const end = h.programme?.endDate ? new Date(h.programme.endDate) : null;
                                    ...
                                    .map((habit) => {
                                        ...
                                        const requiredPerDay = Math.max(1, habit.frequencyPerDay ?? 1);
                                            ...
                                            <div
                                                ...
                                                onClick={() => {
                                                    if (isDisabledForEdit) {
                                                        toast.error(isFutureSelectedDay
