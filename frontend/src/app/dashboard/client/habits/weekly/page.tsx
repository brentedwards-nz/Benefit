...
import { ChevronLeft, ChevronRight, Calendar, ArrowLeft, ChevronsLeft, ChevronsRight } from 'lucide-react';
...
const WeeklyHabitsPageContent = () => {
    ...
    const isSelf = !searchParams.get('clientId') || searchParams.get('clientId') === session?.user?.id;
    ...
    const generateWeeks = (startDate: Date): WeekData[] => {
        ...
        for (let week = 0; week < 4; week++) {
            ...
            for (let day = 0; day < 7; day++) {
                ...
                const dayCompletions = habitCompletions.filter(c => c.completionDate === dateString);
    ...
    const handleHabitToggle = async (programmeHabitId: string, date: Date, completed: boolean) => {
        ...
        const diffPastDays = Math.floor((todayOnly.getTime() - clickedDateOnly.getTime()) / 86400000);
        if (diffPastDays > 3) {
    ...
    const goBackToOverview = () => {
        ...
        router.push(`/dashboard/client/habits${queryParams.toString() ? '?' + queryParams.toString() : ''}`);
    };
        ...
        const fetchData = async () => {
            try {
                ...
                console.log('API Call Details:', {
                    url: `/api/client/habits/completions?startDate=${startDate.toISOString().split('T')[0]}&endDate=${endDate.toISOString().split('T')[0]}&clientId=${clientId}`,
                ...
                console.log('Response object:', completionsResponse);
    ...
    useEffect(() => {
        if (programmeHabits.length > 0 && weeks.length > 0) {
            ...
            if (dateParam) {
                ...
                console.log('Looking for date:', dateParam, 'Target date:', targetDate.toISOString().split('T')[0]);
            ...
            if (targetWeekIndex !== -1) {
                ...
                console.log('Target week days:', targetWeek.days.map(d => d.date.toISOString().split('T')[0]));

                const targetDayInWeek = targetWeek.days.find(day =>
                    day.date.toISOString().split('T')[0] === targetDate.toISOString().split('T')[0]
                );

                console.log('Target day found:', targetDayInWeek ? targetDayInWeek.date.toISOString().split('T')[0] : 'NOT FOUND');
                    ...
                    {isSubmitting && (
                        <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center">
                            <div className="bg-white dark:bg-gray-900 rounded-lg p-6 shadow-lg flex flex-col items-center gap-3">
                                ...
                                <div className="text-sm text-muted-foreground">Saving your update...</div>
                            </div>
                        </div>
                    )}
                </div>
...
const WeeklyHabitsPage = () => {
    return (
        <Suspense fallback={<Loading title="Loading..." description="Preparing weekly habits view..." />}>
