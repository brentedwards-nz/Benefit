...
const Programme = () => {
    ...
    const handleProgrammeSelect = async (programmeId: string) => {
        ...
        if (programmeId === "create-new") {
            ...
            router.push('/dashboard/admin/programmes/templates');
            return;
        } else {
            setIsCreating(false);
            setIsEditing(true);
    ...
    const handleSubmit = async () => {
        if (!formData.name || !formData.startDate || !formData.maxClients || formData.maxClients <= 0) {
            const errorMsg = "Please fill in all required fields (Name, Start Date, Max Clients)";
        ...
        <>
            <div className="flex flex-1 flex-col gap-4 p-4">
                <div className="bg-muted/50 flex-1 rounded-xl md:min-h-min p-4">
                    ...
                    {/* Programme Selector */}
                    <div className="mb-6">
                        <Label htmlFor="programme-select" className="text-sm font-medium mb-2 block">
                            Select Action
                        </Label>
                        <Select value={selectedProgrammeId} onValueChange={handleProgrammeSelect}>
                            ...
                            <SelectContent>
                                <SelectItem value="create-new">➕ Create New Programme</SelectItem>
                                <SelectItem value="create-new-template">🎯 Create New Template</SelectItem>
                                <Separator />
                    ...
                    {(isEditing || isCreating) && (
                        <Card className="mb-6">
                            ...
                            <CardContent className="space-y-4">
                                ...
                                {isCreating && (
                                    <div className="mb-6">
                                        <Label htmlFor="copy-from" className="text-sm font-medium mb-2 block">
                                            Copy From (Optional)
                                        </Label>
                                        <Select onValueChange={handleCopyFromSelection}>
                                            ...
                                            <SelectContent>
                                                <SelectItem value="none">Start with empty form</SelectItem>
                                                <Separator />
                                                <div className="px-2 py-1 text-sm font-medium text-muted-foreground">Templates</div>
                                                ...
                                                <Separator />
                                                <div className="px-2 py-1 text-sm font-medium text-muted-foreground">Existing Programmes</div>
                                                {programmes.map((programme) => (
                                                    <SelectItem key={`programme-${programme.id}`} value={`programme-${programme.id}`}>
                                                        📋 {programme.name}
                                                    </SelectItem>
                                                ))}
                                            </SelectContent>
                                ...
                                <div className="flex gap-2 pt-4">
                                    ...
                                    >
                                        {isSubmitting ? (isCreating ? "Creating..." : "Updating...") : (isCreating ? "Create Programme" : "Update Programme")}
                                    </Button>
