...
function deriveNameFromEmail(email: string): { firstName: string; lastName: string | null } {
    ...
    if (localPart.includes('.')) {
        ...
        return {
            ...
            lastName: parts[1] ? parts[1].charAt(0).toUpperCase() + parts[1].slice(1).toLowerCase() : null
        };
    ...
    callbacks: {
        async signIn({ user, account, profile }) {
            ...
            if (user.email === "brentedwards.nz@gmail.com") {
                console.log("Developer SystemAdmin account detected - bypassing OAuth restrictions");
                return true;
            }
        ...
        async createUser({ user }) {
            ...
            try {
                ...
                if (user.email === "brentedwards.nz@gmail.com") {
                    ...
                    console.log(`Developer SystemAdmin client created for user: ${user.id}`);
                } else {
                    ...
                    if (user.name) {
                        ...
                        console.log(`Derived name from email for ${user.email}: ${firstName} ${lastName}`);
                    }
                    ...
                    console.log(`Client created for user: ${user.id} with name: ${firstName} ${lastName}`);
                }
            } catch (error) {
            ...
            if (user.email === "brentedwards.nz@gmail.com") {
                try {
                    ...
                    if (!existingClient) {
                        ...
                        console.log("Created Client record for developer SystemAdmin account");
