...
export async function getSystemSettings(keys?: string[]): Promise<ActionResult<SystemSetting[]>> {
  try {
    ...
    return { success: false, message: `Failed to fetch system settings: ${message}` };
  }
}
...
export async function createSystemSetting(
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'An unknown error occurred';
    return { success: false, message: `Failed to create system setting: ${message}` };
  }
}
...
export async function updateSystemSetting(
  ...
  const validation = SystemSettingSchema.omit({ id: true }).partial().safeParse(data);
  if (!validation.success) {
    return {
      success: false,
      message: 'Invalid data provided.',
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'An unknown error occurred';
    return { success: false, message: `Failed to update system setting: ${message}` };
  }
}
...
export async function deleteSystemSetting(id: string): Promise<ActionResult<{}>> {
  try {
    ...
    const message = error instanceof Error ? error.message : 'An unknown error occurred';
    return { success: false, message: `Failed to delete system setting: ${message}` };
  }
}
